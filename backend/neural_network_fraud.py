...
def train_neural_network(df: pd.DataFrame, random_search: bool = True, n_iter: int = 20, cv: int = 5, random_state: int = 42):
    df = df.copy()
    ...
    def fit_and_eval(clf: MLPClassifier):
    ...
    if not random_search:
        return fit_and_eval(base_mlp)

    param_dist = {
        'hidden_layer_sizes': [(256, 128), (128, 64), (128,), (64, 32)],
        'alpha': loguniform(1e-6, 1e-2),
        'learning_rate_init': loguniform(1e-4, 1e-2),
        'activation': ['relu', 'tanh'],
        'solver': ['adam'],
        'max_iter': randint(150, 400),
    }
    ...
    best: MLPClassifier = rnd.best_estimator_  # type: ignore
...
def main():
    ap = argparse.ArgumentParser(description="Train Neural Network (MLP) model for Ethereum fraud/phishing detection.")
    ...
    metrics, model, preproc = train_neural_network(df, random_search=not args.no_tune, random_state=args.seed)
